...
export function Hero() {
  ...
  return (
    <section id="hero" aria-labelledby="hero-title" className="relative">
      <HeroStage
        posterAlt={`${profile.name}, ${profile.role}, photographed in low light.`}
      >
        <div className="relative z-10 max-w-[34ch] max-lg:self-end max-lg:pb-[clamp(72px,14vh,140px)]">
          ...
          <h1 id="hero-title" className="t-display-2 mt-5 text-(--text-primary)">
            {profile.name}
          </h1>

          <p className="t-body-lg mt-5 max-w-[42ch] text-(--text-secondary)">
            {profile.positioning}
          </p>
          ...
          <div data-hero-stage className="mt-8 flex flex-wrap gap-3.5">
            ...
            <a
              href="#work"
              className="inline-flex h-12 items-center rounded-full border border-(--border-subtle) px-6 text-[0.9375rem] font-medium text-(--text-primary) transition-colors duration-(--dur-fast) ease-(--ease-standard) hover:border-(--border-interactive) hover:bg-(--hover-fill-subtle)"
            >
              View work
            </a>
          </div>
